...
function reflect(s:StrategyState){
  const batch=s.reflection.buffer.slice(-REFLECT_EVERY);if(batch.length<REFLECT_EVERY)return false
  s.reflection.cycles+=1
  const proposal=batch.filter((_,i)=>i%2===0),evaluation=batch.filter((_,i)=>i%2===1)
  const candidate=candidateFrom(s.weights,proposal,s.reflection.cycles),currentLoss=loss(s.weights,evaluation),candidateLoss=loss(candidate,evaluation),improvement=currentLoss-candidateLoss,changed=diffKeys(s.weights,candidate),kinds=new Set(batch.map(x=>x.kind)).size,evalKinds=new Set(evaluation.map(x=>x.kind)).size
  let accept=improvement>MIN_IMPROVEMENT&&changed.length>0&&kinds>=2&&evalKinds>=2,reason=''
  if(kinds<2){accept=false;reason='整批样本类型过于单一，拒绝晋升'}
  else if(evalKinds<2){accept=false;reason='留出评估集覆盖不足，拒绝晋升'}
  else if(changed.length===0){accept=false;reason='候选与当前策略几乎一致，无需晋升'}
  else if(improvement<=MIN_IMPROVEMENT){accept=false;reason=`留出回放提升不足 ${(improvement*100).toFixed(2)}%，继续保留当前策略`}
  else reason=`候选只用生成集拟合，并在独立留出回放上提升 ${(improvement*100).toFixed(2)}%`
...
export function strategySummary(s:StrategyState){const w=s.weights,r=s.reflection.last;return{toy:`玩${w.toyPlay.toFixed(0)} 好奇${w.toyCuriosity.toFixed(0)} 偏好${w.toyPreference.toFixed(0)} 探索${w.toyNovelty.toFixed(0)}`,social:`社交${w.socialNeed.toFixed(0)} 偏好${w.socialPreference.toFixed(0)} 门槛${w.socialThreshold.toFixed(0)}`,rest:`休息门槛 ${w.restThreshold.toFixed(0)}`,reflection:`反思${s.reflection.cycles}次 · 通过${s.reflection.accepted} / 拒绝${s.reflection.rejected} · ${r.reason}`}}
