...
function hydrateRegistry(raw:unknown,s:V15StrategyState):PolicyRegistryState{
  ...
  let currentVersion=Math.max(1,Math.floor(finite(r.currentVersion,versions[versions.length-1].version)))
  if(!versions.some(v=>v.version===currentVersion))currentVersion=versions[versions.length-1].version
...
function trimVersions(r:PolicyRegistryState){
  ...
  const current=versionByNumber(r,r.currentVersion),fallback=previousStable(r,r.currentVersion)
  let recent=r.versions.slice(-MAX_VERSIONS)
  if(fallback&&!recent.some(v=>v.version===fallback.version))recent[0]=fallback
  if(current&&!recent.some(v=>v.version===current.version))recent[recent.length-1]=current
  r.versions=[...new Map(recent.map(v=>[v.version,v])).values()].sort((a,b)=>a.version-b.version)
}
function freezeCandidateGeneration(s:StrategyState){
  s.reflection.buffer=[]
  s.shadow.challenger=null
  s.shadow.samples=[]
  s.reflection.last={...s.reflection.last,status:'collecting',sampleCount:0,reason:`Champion v${s.policyRegistry.currentVersion} 稳定性观察中，暂缓生成下一代 Challenger`,at:Date.now()}
}
function registerPromotion(s:StrategyState,baseline:number){
  ...
  r.versions.push(v);r.currentVersion=next;r.observation={rewards:[],kinds:[],startedAt:Date.now()};trimVersions(r);freezeCandidateGeneration(s)
...
function markStable(s:StrategyState,current:PolicyVersion,avg:number,kinds:number){
  ...
  r.observation=null;s.reflection.buffer=[];s.shadow.challenger=null;s.shadow.samples=[]
  r.last={status:'stable',currentVersion:current.version,fallbackVersion:current.version,samples:current.observedSamples,avgReward:avg,baselineReward:current.baselineReward,delta,kinds,reason:`Champion v${current.version} 已通过稳定性观察，成为新的回滚点`,at:Date.now()}
...
function observeCurrentVersion(s:StrategyState,kind:DecisionKind,reward:number){
  const r=s.policyRegistry,o=r.observation;if(!o)return false
  const current=versionByNumber(r,r.currentVersion);if(!current||current.status!=='observing'){r.observation=null;return false}
  ...
  if(o.rewards.length<OBSERVE_SAMPLES)return false
  if(kinds<MIN_KINDS){r.last={...r.last,reason:`观察样本类型不足 ${kinds}/${MIN_KINDS}，继续保持观察并等待更丰富行为`};o.rewards=o.rewards.slice(-Math.floor(OBSERVE_SAMPLES/2));o.kinds=o.kinds.slice(-Math.floor(OBSERVE_SAMPLES/2));return false}
  if(delta<-MAX_REGRESSION&&fallback){rollback(s,current,fallback,avg,kinds);return true}
  markStable(s,current,avg,kinds);return false
}

export function reportDecisionReward(s:StrategyState,t:DecisionTrace|undefined|null,reward:number){
  const settling=willSettle(t),settledReward=settling&&t?finalReward(t,reward):0,wasObserving=Boolean(s.policyRegistry.observation),baselineBefore=recentBaseline(s),beforeWeights=cloneWeights(s.weights)
  if(wasObserving)freezeCandidateGeneration(s)
  const changed=reportV15(s,t,reward)
  if(wasObserving){
    freezeCandidateGeneration(s)
    const rolledBack=settling&&t?observeCurrentVersion(s,t.kind,settledReward):false
    return rolledBack
  }
