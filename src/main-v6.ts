...
for (let i = 0; i < 3; i += 1) {
  ...
  const b = a.clone()
  a.position.x = -.07; b.position.x = .07
  g.add(a,b)
...
const crossV = box(.08,2.25,.08,C.white,.04); crossV.position.set(-2.75,3.35,-5.1)
world.add(crossH,crossV)

// 家具
const shelf = box(3.35,1.52,.82,0xd8b68f,.18); shelf.position.set(3.78,.78,-5.03); world.add(shelf)
const couch = new THREE.Group()
const seat = box(2.65,.56,1.35,0xc9b6d8,.28,.92); seat.position.y=.52
const back = box(2.65,1.28,.5,0xb3a0c6,.28,.93); back.position.set(0,1.12,-.47)
couch.add(seat,back); couch.position.set(4.42,.08,2.72); couch.rotation.y=-.22; world.add(couch)
...
const lampShade = new THREE.Mesh(new THREE.CylinderGeometry(.3,.52,.64,30),mat(C.butter)); lampShade.position.set(5.45,1.68,-3.75)
...
const slideEnd = new THREE.Vector3(-4.85,.2,-1.78)
{
  ...
  for (const x of [-.9,.9]) {
    const tower = box(.72,2.35,.72,x<0?C.peach:C.sky,.2); tower.position.set(x,1.17,-.18); castle.add(tower)
    const cap = new THREE.Mesh(new THREE.ConeGeometry(.55,.78,4),mat(x<0?C.coral:C.blue)); cap.rotation.y=Math.PI/4; cap.position.set(x,2.72,-.18); castle.add(cap)
  }
  ...
  const slide = new THREE.Mesh(new THREE.TubeGeometry(slideCurve,40,.24,14,false),mat(C.butter,.48)); slide.castShadow=true; castle.add(slide)
  castle.position.set(-3.8,.05,-3.55); world.add(castle)
...
;[C.coral,C.butter,C.sky,C.mint,C.lavender].forEach((color,i)=>{
  const b=box(.48,.48,.48,color,.09); const homes=[[-.45,.25,0],[.05,.25,.08],[.5,.25,0],[-.2,.75,.04],[.3,.75,.02]]
  b.position.set(homes[i][0],homes[i][1],homes[i][2]); b.userData.status='home'; b.userData.stackIndex=-1; blocks.push(b); blockGroup.add(b)
})
...
const ballGroup=new THREE.Group(); const ballVelocity=new THREE.Vector3()
const ball=sphere(.36,0xf2bf79,.48); ballGroup.add(ball,shadow(.34,.14)); ballGroup.position.set(2.05,.47,.85); registerToy('ball','软球','○',ballGroup,()=>ballGroup.position.clone().setY(.2))

const jellyGroup=new THREE.Group(); const jellyHome=new THREE.Vector3(.55,.04,3.62); jellyGroup.position.copy(jellyHome)
const jellyGeo=new THREE.SphereGeometry(.64,36,28); const jellyBase=new Float32Array(jellyGeo.attributes.position.array as ArrayLike<number>)
...
jellyMesh.position.y=.72; jellyGroup.add(jellyMesh,shadow(.62,.15)); registerToy('jelly','果冻','●',jellyGroup,()=>new THREE.Vector3(.3,.2,2.7))
let jellyImpulse=0,jellyVel=0; const jellyDrift=new THREE.Vector3(),jellyDriftVel=new THREE.Vector3()

const trainGroup=new THREE.Group(),trainBody=new THREE.Group(); trainGroup.position.set(-.65,.02,-2.6)
const trainBase=box(1.35,.5,.66,C.coral,.13); trainBase.position.y=.48; trainBody.add(trainBase); trainGroup.add(trainBody)
const track=new THREE.Mesh(new THREE.TorusGeometry(1.66,.045,10,64),mat(C.cocoa)); track.rotation.x=Math.PI/2; track.scale.z=.63; track.position.y=.14; trainGroup.add(track); registerToy('train','小火车','◉',trainGroup,()=>new THREE.Vector3(.35,.2,-1.95))

const musicGroup=new THREE.Group(); const musicBars:THREE.Mesh[]=[]; musicGroup.position.set(4.2,.04,-.6)
...
const booksGroup=new THREE.Group(); booksGroup.position.set(-5,.02,-.35)
...
function createAgent(name:string,color:number,skin:number,hair:number,start:THREE.Vector3,accent:number){
  ...
  const torso=box(.68,.92,.5,color,.26);torso.position.y=1.02;root.add(torso)
  const bib=box(.48,.54,.515,accent,.14);bib.position.set(0,.98,.025);root.add(bib)
  const head=sphere(.45,skin,.78);head.position.y=1.78;root.add(head)
  const cap=new THREE.Mesh(new THREE.SphereGeometry(.46,30,18,0,Math.PI*2,0,Math.PI*.54),mat(hair,.95));cap.position.y=1.9;root.add(cap)
  const eyes:THREE.Mesh[]=[];for(const x of [-.16,.16]){const e=sphere(.038,C.dark);e.position.set(x,1.79,.42);eyes.push(e);root.add(e)}
  const leftArm=new THREE.Group(),rightArm=new THREE.Group();leftArm.position.set(-.4,1.3,0);rightArm.position.set(.4,1.3,0)
  for(const p of [leftArm,rightArm]){const a=cylinder(.09,.72,skin);a.position.y=-.35;p.add(a);root.add(p)}
  const leftLeg=new THREE.Group(),rightLeg=new THREE.Group();leftLeg.position.set(-.18,.62,0);rightLeg.position.set(.18,.62,0)
  ...
  const hand=new THREE.Group();hand.position.set(0,1.02,.65);root.add(hand)
  group.userData.agentName=name;world.add(group)
  return{name,group,root,leftArm,rightArm,leftLeg,rightLeg,hand,eyes,path:[],pathIndex:0,state:'idle' as AgentState,timer:0,targetToy:null,held:null,assignedBlock:null,castlePhase:-1,castleTime:0}
}
const tao=createAgent('陶陶',C.sage,0xe6b18d,0x56463f,new THREE.Vector3(-.35,.2,.55),0x728b84)
const momo=createAgent('沫沫',C.lavender,0xe8b995,0x4d403c,new THREE.Vector3(1,.2,1.15),0x7d7392)
const agents=[tao,momo]

let paused=false,isNight=false,cinematic=true;let follow:Agent|null=null;let idleSince=performance.now(),lastManual=performance.now();const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches
...
function startRecovery(manual=false){
  socialMode='none';const pool=recoverableBlocks();if(!pool.length){statusText.textContent='积木已经收拾好啦';idleSince=performance.now();return}
  const first=pool[0],second=pool[1]??null;tao.assignedBlock=first;moveAgent(tao,blockWorldPos(first).setY(.2),'pickup',toys.find(t=>t.key==='blocks')??null)
  if(second){momo.assignedBlock=second;moveAgent(momo,blockWorldPos(second).setY(.2),'pickup',toys.find(t=>t.key==='blocks')??null)}else{moveAgent(momo,buildSpot.clone().add(new THREE.Vector3(.65,0,.2)),'wait',null)}
  statusText.textContent=manual?'一起把积木收回来':'TA 们发现积木散了，先收拾房间';autoBadge.textContent='分工收拾积木'
}
...
function startToy(toy:Toy){
  if(toy.key==='blocks'){startRecovery(true);return}if(toy.key==='ball'){startBallPass(true);return}if(toy.key==='castle'){startCastle();return}
  socialMode='none';moveAgent(tao,toy.target(),'playing',toy);moveAgent(momo,toy.target().clone().add(new THREE.Vector3(.65,0,.35)),'playing',toy);statusText.textContent=`一起去玩${toy.label}`;autoBadge.textContent=`双人 · ${toy.label}`
}
...
for(const [label,emoji,fn] of [['追逐','↝',startTag],['招手','⌁',startWave]] as const){const b=document.createElement('button');b.className='toy-button';b.innerHTML=`<span>${emoji}</span><b>${label}</b>`;b.onclick=()=>{lastManual=performance.now();fn()};toyButtons.appendChild(b)}
...
function animateAgent(a:Agent,delta:number,elapsed:number){if(walk(a,delta,elapsed)||castleAnim(a,delta))return;a.timer+=delta;const sway=Math.sin(elapsed*4+(a===momo?1.2:0))
  if(a.state==='pickup'){
    a.leftArm.rotation.x=-.9;a.rightArm.rotation.x=-1.15
  ...
  if(a.state==='stack'){
    a.leftArm.rotation.x=-1.05;a.rightArm.rotation.x=-1.05
  ...
  if(a.state==='tag'){
    ...
    a.leftArm.rotation.x=-sway*.5;a.rightArm.rotation.x=sway*.5
    return
  }
  if(a.state==='wave'){
    a.rightArm.rotation.z=-1.55;a.rightArm.rotation.x=Math.sin(elapsed*7)*.35-.3;a.group.lookAt((a===tao?momo:tao).group.position);return
  }
  if(a.state==='wait'){
    a.leftArm.rotation.x=Math.sin(elapsed*1.5)*.06;a.rightArm.rotation.x=-a.leftArm.rotation.x;return
  }
  if(a.state==='playing'){
    a.leftArm.rotation.x=-.45+sway*.25;a.rightArm.rotation.x=-.45-sway*.25;if(a.targetToy?.key==='music')a.rightArm.rotation.x=-1.05+Math.sin(elapsed*8)*.35;if(a.targetToy?.key==='jelly'&&Math.sin(elapsed*4+a.timer)>.94)pokeJelly(.22,a.group.position);if(a.timer>4.5){a.state='idle';a.targetToy=null;resetPose(a);idleSince=performance.now()}return
  }
